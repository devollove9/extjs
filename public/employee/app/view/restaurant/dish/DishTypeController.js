 ...
 */
Ext.define('517Employee.view.restaurant.dish.DishTypeController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.employee-restaurant-dish-type-controller',
    requires: [
    ...
    NewType:function( grid , rowIdx , columnIdx , buttonIcon , clickEvent , recordLine , tr ) {
        if ( ! Ext.getCmp( 'Employee-Restaurant-Dish-Category' ).getSelectionModel().hasSelection() ) {
            ...
            this.CreateNewWindow( Ext.getCmp( 'Employee-Restaurant-Dish-Category' ).getSelectionModel().getSelection()[ 0 ].data );
        }
    },
    ...
    DeleteType:function(){
        Ext.Msg.alert( "Warning" , "Api 暂时没有 '删除 Dish Type' 功能" );
    ...
    CreateWindow: function( record ) {
        if ( Ext.getCmp( 'Employee-Restaurant-Dish-Type' ).gridEditing == true ) {
            ...
            var win = Ext.create('Ext.window.Window', {
                ...
                xtype: 'employee-restaurant-dish-type-window',
                title: 'Dish Type - ' + record.data.name + '(' + record.data.nameEn + ')',
                layout:'fit',
                ...
                items: [
                    {
                        ...
                        items: [
                            ...
                            },
                            {
                                ...
                                store:typeBusinessHourStore,
                                localstoreId:typeBusinessHourStore.id,
                ...
                listeners:{
                    'close':function( win ){
                        Ext.getCmp( 'Employee-Restaurant-Dish-Type' ).gridEditing = false;
            ...
            Ext.getCmp( 'Employee-Restaurant-Dish-Type' ).addOpenedWindow( win );
            win.show();
        }
    },
    ...
    CreateNewWindow: function( record ) {
        ...
        if ( Ext.getCmp( 'Employee-Restaurant-Dish-Type' ).gridEditing == true ) {
            ...
            typeBusinessHourStore.add( businessHour );

            var win = Ext.create('Ext.window.Window', {
                extend:'Ext.form.Panel',
                xtype: 'employee-restaurant-dish-type-window',
                title: 'New Dish Type - ' + record.name,
                layout:'fit',
                width:500 , minWidth:400 , maxWidth:500 ,
                height:470 , minHeight:470 , maxHeight:470 ,
                autoScroll:false , resizable:true ,
                            ...
                            },
                            {
                                ...
                                handler:function( field , rowIndex ) {
                                    ...
                                    if ( unavailable == false ) {
                                        Ext.Msg.show({
                                            title:'Add Type?',
                                            msg: 'You will add type: <br/>  '+ post_string + ' <br/>Would you like to add the type?',
                                            buttons: Ext.Msg.YESNO,
                                            fn: function(btn,text){
                                                if ( btn == 'yes' ) {
                                                    var typeInfo = new Object();
    ...
    NewBusinessHour:function( button , clickEvent ) {
        ...
        if ( typeWindow.gridEditing == true ) {
            ...
            var win = Ext.create('Ext.window.Window', {
                ...
                listeners:{
                    'close':function( win ) {
                        typeWindow.gridEditing = false;
                    }
                }

            });
            typeWindow.addOpenedWindow( win );
            win.show();
        }
    },
    ...
    DeSelectAll:function( field ) {
        ...
        Ext.getCmp( 'Employee-Restaurant-Dish-Type' ).closeWindowPopUp();
    }
});
